...
/*exports.smsg = (conn, m, hasParent) => {
     ...
     */
     m.reply = async (text, chatId, options) => {
        ...
        conn.reply(chatId ? chatId : m.chat, text, m, { contextInfo: { mentionedJid: conn.parseMention(text), externalAdReply: { title: 'Kisuke Botz', body: wm, sourceUrl: 'https://instagram.com/nansoffc', thumbnail: data }}, options })
    }
    m.name = m.pushName || conn.getName(m.sender)
